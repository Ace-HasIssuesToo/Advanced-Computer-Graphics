...
Mesh* MeshBuilder::GenerateSkyPlane(const std::string &meshName, Color color, int slices, float PlanetRadius, float AtmosphereRadius, float hTile, float vTile)
{
	...
	//Checking if the number of slices is within 1 - 256 
	(slices < 1) ? (slices = 1) : (slices > 256) ? (slices = 256) : (slices);

	//Calculating values needed for skyplane
	//Finding Plane Size
	//float planeSize = 2.0f * sqrtf((AtmosphereRadius * AtmosphereRadius) - (PlanetRadius * PlanetRadius));
	float planeSize = 2.0f * sqrtf(pow(AtmosphereRadius, 2) - pow(PlanetRadius, 2));
	...
	for (int z = 0; z <= slices; ++z)
	{
		...
		{
			float xDistance = (-0.5f * planeSize) + ((float)x * delta);
			float zDistance = (-0.5f * planeSize) + ((float)z * delta);

			float xHeight = pow(xDistance, 2) / AtmosphereRadius;
			float zHeight = pow(zDistance, 2) / AtmosphereRadius;
			float height = xHeight + zHeight;

			Vertex tV; //This is a temporary Vertex
			tV.pos.x = xDistance;
			tV.pos.y = 0.0f - height; //<-- Different
			//tV.pos.y = -height;		//<-- This is also Different
			tV.pos.z = zDistance;
